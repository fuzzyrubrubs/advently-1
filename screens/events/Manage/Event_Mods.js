...
import { make_event_mod, remove_event_mod } from '../../../firebase/methods/Event_Functions';


function Event_Mods(props){   
    ...
    useFocusEffect(
        React.useCallback(() => {
            const fetch_data = async () => {
                const users_data = await Promise.all(event_data.members.filter(item => item !== event_data.admin).map(async (member) => {
                    return await get_user_data(member);
                }))
                set_mods_data(users_data.filter(item => event_data.mods.includes(item.id)))
                set_members_data(users_data.filter(item => !event_data.mods.includes(item.id)));
            }
            fetch_data();
        }, [event_data])
      );
    ...
    const make_mod_handler = (item) => {
        make_event_mod(item.id, event_data)
    }

    const remove_mod_handler = (item) => {
        remove_event_mod(item.id, event_data)
    }
